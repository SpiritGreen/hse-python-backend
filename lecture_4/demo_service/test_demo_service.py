...
from lecture_4.demo_service.api.utils import UserServiceDep, AuthorDep, AdminDep
...
def test_user_service_register():
    ...
    assert entity.info.birthdate == datetime.fromtimestamp(0.0)
    assert entity.info.role == UserRole.USER

def test_user_service_register_duplicate_username():
    user_service = UserService(password_validators=[password_is_longer_than_8])
    user_info = UserInfo(
        username="test_user",
        name="Test User",
...
def test_user_service_register_invalid_password():
    user_service = UserService(password_validators=[password_is_longer_than_8])
    user_info = UserInfo(
        username="test_user",
        name="Test User",
...
def test_user_service_get_by_username():
    user_service = UserService(password_validators=[password_is_longer_than_8])
    user_info = UserInfo(
        username="test_user",
        name="Test User",
    ...
    entity = user_service.register(user_info)
    retrieved_entity = user_service.get_by_username("test_user")
    assert retrieved_entity.uid == entity.uid
    assert retrieved_entity.info.username == entity.info.username

def test_user_service_get_by_id():
    user_service = UserService(password_validators=[password_is_longer_than_8])
    user_info = UserInfo(
        username="test_user",
        name="Test User",
    ...
    entity = user_service.register(user_info)
    retrieved_entity = user_service.get_by_id(entity.uid)
    assert retrieved_entity.uid == entity.uid
    assert retrieved_entity.info.username == entity.info.username

def test_user_service_get_by_nonexistent_username():
    user_service = UserService(password_validators=[password_is_longer_than_8])
    retrieved_entity = user_service.get_by_username("nonexistent_user")
    assert retrieved_entity is None

def test_user_service_get_by_nonexistent_id():
    user_service = UserService(password_validators=[password_is_longer_than_8])
    retrieved_entity = user_service.get_by_id(1)
    assert retrieved_entity is None

def test_user_service_grant_admin():
    user_service = UserService(password_validators=[password_is_longer_than_8])
    user_info = UserInfo(
        username="test_user",
        name="Test User",
    ...
    updated_entity = user_service.get_by_id(entity.uid)
    assert updated_entity.info.role == UserRole.ADMIN

def test_user_service_grant_admin_nonexistent_user():
    user_service = UserService(password_validators=[password_is_longer_than_8])
    with pytest.raises(ValueError):
        user_service.grant_admin(1)

def test_password_is_longer_than_8_valid():
    assert password_is_longer_than_8("longpassword123") is True
...
def test_add_exception_handler():
    ...
    assert exception_handlers[ValueError] == utils.value_error_handler
